...
public class GoodsService extends CommonService{
	...
	public Result loadIndexGoodsList(){
		...
		//加载首页Banner
		List<Map<Object,Object>> banners=getSonDicts("home_banner").stream().map(g->{
		...
		return putResult(DataUtil.mapOf("welcomes", welcomes, "lives", lives, "shops", shops, "healths", healths, "banners", banners));
	}
